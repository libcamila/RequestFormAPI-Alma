...
?>
   ...
   body, p, div {
	...
	$pickupLocation = '';
	$pickupLibrary = !empty($_REQUEST['pickupLibrary']) ? $_REQUEST['pickupLibrary'] : '';
...
$makeRequest = json_decode('{"request_type":"HOLD","description":"' . $itemDescription . '","pickup_location_type":"LIBRARY","pickup_location_library":"'.$pickupLibrary.'","pickup_location_circulation_desk":"","pickup_location_institution":"","material_type":{"value":""},"comment":"Item to be picked up at '.$pickupLocation.'"}', true);
...
elseif (!empty($_REQUEST['req_type']) && preg_match('/summit/i', $_REQUEST['req_type'])) {
	...
	$title       = !empty($_REQUEST['title']) ? stripslashes(trim(urldecode($_REQUEST['title']))) : '';
	$isn         = !empty($_REQUEST['isn']) ? urldecode(trim($_REQUEST['isn'])) : '';
	$req_type    = !empty($_REQUEST['type']) ? $_REQUEST['type'] : '';
	$oclc_num    = !empty($_REQUEST['oclcnum']) ? trim($_REQUEST['oclcnum']) : (!empty($_REQUEST['oclc']) ? trim($_REQUEST['oclc']) : '');
	$oclcnum     = $oclc_num;
	...
	$pickup_location['value']            = $pickupLibrary;
	$pickup_location['desc']             = $pickupLibraries[$pickupLibrary];
	...
	$request['note'] .= " Request submitted via API.";
	...
	{
		print 'REQUEST NOT PLACED FOR TESTING!!!';
