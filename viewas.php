...
?>
        <script>
            ...
            $('.openbtn').click(function() {
                ...
                let viewAs = '<?php print !empty($_REQUEST['viewas']) ? $_REQUEST['viewas'] : ""; ?>';
                let prependContent = `<div id="adminBanner" class="alert alert-dismissible alert-info" style="margin-top:-2em;margin-left:-2em;margin-right:-2em;"><button onclick="showButton()" type="button" class="close" data-dismiss="alert">&times;</button><strong>Welcome to Admin Version 1.0!</strong><div id="adminContent"><h3>Permissions</h3><div><form method="post" action="<?php print $_SERVER['PHP_SELF']; ?>">View as: &nbsp;<input type="text" name="viewas" size="30" style="max-width:10em;" value="${viewAs}">`;
                <?php
                if (!empty($_REQUEST)) {
                    foreach ($_REQUEST as $k => $v) {
                        if ($k != 'viewas' && $k != 'assoc_prog_name') {
                            print "prependContent += '<input type=\"hidden\" name=\"{$k}\" value=\"{$v}\">';";
                        }
                        unset($k, $v);
                    }
                }
                if (preg_match('/view[a-zA-Z]*.php/i', $_SERVER['PHP_SELF'])) {
                    print "prependContent += '&nbsp;Edit request? &nbsp;Yes&nbsp;<input type=\"radio\" name=\"edit\" value=\"yes\">&nbsp;&nbsp;No&nbsp;<input type=\"radio\" name=\"edit\" value=\"no\" checked>';";
                }
                ?>
                prependContent += '&nbsp; &nbsp;<input type="submit" name="Submit"></form></div></div></div>';
                $.when($('body').prepend(prependContent)).done(function() {
